...
function animateHeader() {
  ...
  if (calcul > 90) {
    mainNode.style.marginTop =
      initialHeaderHeight + window.scrollY * 0.3 + "px";
  }

  //Gestion des etapes
  let contentDistance = document
...
function updateFollow(entries, observer) {
  for (const entry of entries) {
    ...
    if (ratio > observer.thresholds[0] && ratio < 1) {
      ...
      const concernedLinks = document.querySelectorAll(
        `.header__menu__item a[href="#${id}"]`
      );
