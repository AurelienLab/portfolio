...
document.addEventListener("DOMContentLoaded", () => {
  if (darkmode) {
    ...
    for (const button of darkmodeButtons) {
      button.innerHTML =
        '<a href="" aria-label="Activer le thème clair"><i class="fa-solid fa-sun"></i></a>';
    }
  }
  initMenu();
  ...
  getData()
    .then(() => {
      ...
      generateSitesList(
        filtrerSites(sites, categories.find((cat) => cat.slug === "all").id)
      );
...
function insertAboutMe(page) {
  ...
  document.getElementById("about-me__content").innerHTML =
  ...
  for (const site of sites) {
    ...
    let html = `<div class="card__container initial">
      ...
      site.altText
                            ...
                            <div class="front__title front__title--${
                              categories.find(
                                (cat) =>
                                  site.categories.includes(cat.id) &&
                                  cat.slug !== "all"
                              ).slug
                            }" data-lb-no-trigger>
                                <div class="shape"></div>
    ...
    if (site.url) {
      html += `<a href="${
        site.url
      }" class="popover" aria-label="Lien vers le site" data-lb-no-trigger data-popovercontent="${site.url.replace(
        /^https?:\/\//,
        ""
...
function handleSitesClick() {
  ...
  for (const card of cards) {
    card.addEventListener("click", function (e) {
      if (
        e.target.hasAttribute("data-lb-no-trigger") ||
        e.target.parentNode.hasAttribute("data-lb-no-trigger")
      )
        return;
      const site = sites.find(
        (site) => site.id === parseInt(e.currentTarget.dataset.siteid)
      );
      ...
      createLightbox(
        site.title.rendered,
        site.content.rendered + bodyHTML,
        footerHTML
      );
      initPopovers();
    });
...
const toggleDarkMode = (e) => {
  ...
  if (darkmode) {
    ...
    buttonIconHTML =
      '<a href="#" aria-label="Activer le thème sombre"><i class="fa-solid fa-moon"></i></a>';
  } else {
    document.getElementsByTagName("body")[0].classList.add("dark");
    buttonIconHTML =
      '<a href="#" aria-label="Activer le thème clair"><i class="fa-solid fa-sun"></i></a>';
...
const burgerMenuItems = document.querySelectorAll(
  ".burger__container .header__menu__item:not(.darkmode-toggle)"
);
